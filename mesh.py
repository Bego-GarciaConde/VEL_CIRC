...
class Mesh:
    """Defines the mesh (situated at Z=0) in which the acceleration is computed"""
    ...
    def calculate_vcirc(self):
        ...
        data = {'X': self.x, 'Y': self.y, 'Z': self.z, 'R': self.R, 'Phi': self.phi, 'ar_dm': ar_dm,
                'a_stars': ar_stars,
    ...
    def plot_Vcirc(self):
        size = 5
        ancho = limit - 20
        fig, ax = plt.subplots(1, 2, sharex=False, sharey=False, figsize=(11, 4))
        az = ax[0].scatter(self.x, self.y, marker='s', c=self.Vcirc,
                           cmap="rainbow", s=size, vmin=150, vmax=275)

        divider = make_axes_locatable(ax[0])
        cax = divider.append_axes('right', size='5%', pad=0.3)
        cb = fig.colorbar(az, cax=cax, orientation='vertical')
        cb.set_label(label="V$_{circ}$ km/s", fontsize=14)

        ax[0].set_xlabel("X [kpc]", fontsize=14)
        ax[0].set_ylabel("Y [kpc]", fontsize=14)
        ax[0].set_xlim(-ancho, ancho)
        ax[0].set_ylim(-ancho, ancho)
        # ax[0].set_box_aspect(1)

        ax[1].set_xlabel("R [kpc]", fontsize=14)
        ax[1].set_ylabel("V$_{circ}$ [km/s]", fontsize=14)

        radarm = np.arange(0, limit, 0.5)
        indr = np.digitize(self.R, radarm) - 1
        Vcirc_profile = np.zeros(np.max(indr), dtype=np.float64)
        R_profile = np.zeros(np.max(indr), dtype=np.float64)

        for i in range(np.max(indr)):
            # Vcirc_i=mesh.Vcirc[indr==i]
            Vcirc_profile[i] = np.mean(self.Vcirc[indr == i])
            R_profile[i] = np.mean(self.R[indr == i])

        ax[1].plot(R_profile, Vcirc_profile, label="$V_{circ}$ ($a_{R}$)", color="black", lw=1.5)
        plt.subplots_adjust(left=0, bottom=0., right=1, top=1, wspace=0.4, hspace=0.05)
        plt.savefig(f"Figures/{self.name}.png", format='png', dpi=100, bbox_inches='tight')
